#!/bin/bash
# DreamZero DROID Full Fine-Tuning Script with Wan2.1-I2V-14B-480P backbone
#
# Usage:
#   bash scripts/train/droid_training_full_finetune_wan21.sh
#
# Prerequisites:
...
# For concurrent runs use: OUTPUT_DIR=.../dreamzero_droid_wan21_full_finetune_$(date +%Y%m%d_%H%M%S)
#
...
OUTPUT_DIR=${OUTPUT_DIR:-"/scratch/sombit_dey/dreamzero_ckpts/dreamzero_droid_wan21_full_finetune"}
...
# Wan2.1-I2V-14B-480P checkpoint (includes CLIP image encoder, T5 text encoder, and VAE)
WAN21_CKPT_DIR=${WAN21_CKPT_DIR:-"/scratch/sombit_dey/dreamzero_ckpts/Wan2.1-I2V-14B-480P"}
TOKENIZER_DIR=${TOKENIZER_DIR:-"/scratch/sombit_dey/dreamzero_ckpts/umt5-xxl"}
# =============================================

# ============ AUTO-DOWNLOAD WEIGHTS ============
if [ ! -d "$WAN21_CKPT_DIR" ] || [ -z "$(ls -A "$WAN21_CKPT_DIR" 2>/dev/null)" ]; then
    echo "Wan2.1-I2V-14B-480P not found at $WAN21_CKPT_DIR. Downloading from HuggingFace..."
    huggingface-cli download Wan-AI/Wan2.1-I2V-14B-480P --local-dir "$WAN21_CKPT_DIR"
fi
